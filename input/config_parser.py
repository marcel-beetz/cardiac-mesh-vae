...
def set_default_parameters(config):
    ...
    config.set('Input Output', 'output_dir', '')
    config.set('Input Output', 'template_filepath', './template/template.ply')
    ...
    config.set('Learning Parameters', 'batch_size', 8)
    config.set('Learning Parameters', 'learning_rate', 0.001)
    config.set('Learning Parameters', 'learning_rate_decay', 0.99)
    config.set('Learning Parameters', 'weight_decay', 0.0001)
...
def read_config(fname):
    ...
    config_parms['template_filepath'] = config.get('Input Output', 'template_filepath')
    config_parms['output_dir'] = config.get('Input Output', 'output_dir')
...
if __name__ == '__main__':
    ...
    config_fname = os.path.join(pkg_path, 'settings.cfg')
